...
<fullquery name="get_relationships">
      <querytext>
select rel_id, other_name, other_party_id, role_singular, role_plural, rel_type, sort_name
  from ( select CASE WHEN object_id_one = :party_id THEN contact__name(object_id_two,'t') ELSE contact__name(object_id_one,'t') END as sort_name,
 ...
 order by upper(role_singular) asc, upper(sort_name)
      </querytext>
</fullquery>
