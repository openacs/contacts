...
ad_proc -public contacts::import::csv {
	...
	if {![exists_and_not_null values(E-Mail)]} {
	    ...
	    set person_email $values(E-Mail)
	}
	...
	if {![exists_and_not_null values(Firma)]} {
	    set organization_name "$first_names $last_name"
	} else {
	...
	if {[llength $existing_organizations] == 0} {
	    ...
	    set organization_id [organization::new -name $organization_name -email $values(E-Mail) -url $values(Internet)]
	    ...
	    # Add to the lexware group
	    set rel_id [db_string insert_rels2 { select acs_rel__new (NULL::integer,'organization_rel',:lexware_group_id,:organization_id,NULL,NULL,NULL) as org_rel_id }]
	    db_dml insert_state2 { insert into membership_rels (rel_id,member_state) values (:rel_id,'approved') }
		...
		if {![group::party_member_p -group_id $lexware_group_id -party_id $organization_id]} {
		    # Add to the lexware group
		    set rel_id [db_string insert_rels2 { select acs_rel__new (NULL::integer,'organization_rel',:lexware_group_id,:organization_id,NULL,NULL,NULL) as org_rel_id }]
		    db_dml insert_state2 { insert into membership_rels (rel_id,member_state) values (:rel_id,'approved') }
		}
	    ...
	    set person_id [db_string get_person_by_names {
		...
		ns_log Notice "contacts::import::leads => Person created $first_names $last_name ($person_email): $person_id"
